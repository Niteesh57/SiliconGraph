...
std::string generateSelectorC(const std::vector<CompiledGraph>& family) {
  ...
  oss << "// ARM AI Compiler — Auto-generated Graph Selector\n"
      ...
      << "  uint8_t  execution_backend; // 0=CPU, 1=Vulkan\n"
  ...
  for (uint32_t i = 0; i < (uint32_t)family.size(); ++i) {
    ...
    oss << "  { "
        ...
        << (cg.conditions.execution_backend == ir::ExecUnit::GPU ? 1 : 0) << ", "
        << i << ", "
  ...
  oss << "uint32_t select_graph(\n"
      ...
      << "  int vulkan_benchmark_wins,\n"
      << "  int media_requires_medium_or_higher_precision)\n"
      << "{\n"
      << "  uint32_t best_idx = UINT32_MAX;\n"
      << "  float    best_score = 1e30f;\n"
      << "  (void)battery_pct; // battery changes response length, not precision\n"
      << "\n"
      ...
      << "      continue;\n"
      << "\n"
      << "    // INT4/INT2/FP8 variants are text-only when media is present.\n"
      << "    if (media_requires_medium_or_higher_precision &&\n"
      << "        (e->quant_dtype == 8 || e->quant_dtype == 9 ||\n"
      << "         e->quant_dtype == 3 || e->quant_dtype == 4))\n"
      << "      continue;\n"
      << "\n"
      << "    // SoC mismatch penalty\n"
      << "    if (e->soc_id != soc_id && e->soc_id != 0xFF) score += 200.0f;\n"
      << "\n"
      << "    // Vulkan graphs are usable only after an end-to-end benchmark win.\n"
      << "    if (e->execution_backend == 1 && !vulkan_benchmark_wins) continue;\n"
      << "\n"
